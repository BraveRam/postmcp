...
program
  ...
  .option('-l, --lang <language>', 'Target language: ts (TypeScript) or py (Python/FastMCP)', 'ts')
...
program
  ...
  .option('--client <name>', 'Target client: cursor, claude, windsurf, or all (default: all)', 'all')
  ...
  .action((spec, opts) => {
    exportCommand(spec, opts).catch((err) => {
      console.error(pc.red(`Fatal error: ${err.message}`));
      process.exit(1);
    });
  });
