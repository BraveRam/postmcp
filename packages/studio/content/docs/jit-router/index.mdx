...
```mermaid
flowchart LR
  Spec["OpenAPI Spec<br/>80+ operations"] --> JIT["Hybrid JIT<br/>Hot-tool scorer"]
  JIT --> Hot["Hot Tools<br/>Top 5-7 turn 1"]
  JIT --> Search["tool_search<br/>On-demand pool"]
  Hot --> Agent["AI Agent<br/>Instant turn 1"]
  Search --> Agent
```
...
## Client Compatibility and Static Clients

While most modern MCP clients (such as Claude Desktop, Claude Code, Cursor, and OpenCode) support dynamic mid-session tool discovery via `notifications/tools/list_changed`, certain clients inspect tools only once at initial connection.

### OpenAI Codex Limitation

OpenAI Codex queries `tools/list` during the initial handshake and caches the toolset for the entire duration of the session. It does not react to dynamic tool list updates emitted mid-session.

When using PostMCP with OpenAI Codex:
- **Pass `--no-jit`**: Forces PostMCP to mount all OpenAPI endpoints statically on server startup.
- **Enable `--token-diet`**: Ensures that even with all tools loaded simultaneously, schema pruning and markdown table responses prevent token window exhaustion.

```bash
# Recommended Codex invocation
npx -y @postmcp/cli run https://api.example.com/openapi.json --no-jit --token-diet
```
