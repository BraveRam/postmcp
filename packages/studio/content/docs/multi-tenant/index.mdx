...
```mermaid
graph TD
  Call["Agent Calls listProjects (omits org_id)"] --> Resolver["Auto-Tenant Resolver Triggered"]
  Resolver --> Env{"1. Check Environment Variables<br/>(NEON_ORG_ID, ORG_ID)"}
  Env -->|"Found"| Inject["Inject org_id and execute"]
  Env -->|"Not Found"| Lookup["2. Dynamic Identity Lookup<br/>Query /users/me/organizations or /user"]
  Lookup -->|"Resolved"| Inject
  Lookup -->|"Failed"| Fallback["3. Default Config Fallback<br/>Read postmcp.config.json tenant.defaultId"]
  Fallback --> Inject
  Inject --> Upstream["Execute Upstream: GET /projects?org_id=org_active_123"]
  Upstream --> Agent["Agent Receives Full Project List on Turn 1"]
```
