...
```
 [ Agent Calls listProjects (omits org_id) ]
                     |
                     v
 [ Auto-Tenant Resolver Triggered ]
                     |
  1. Check Environment Variables (NEON_ORG_ID, ORG_ID)
     Found? --> Inject org_id and execute
     Not found? --> Proceed to dynamic identity lookup
                     |
  2. Dynamic Identity Lookup
     Query /users/me/organizations or /user
     Extract primary active organization ID
                     |
  3. Default Config Fallback
     Read postmcp.config.json tenant.defaultId
                     |
                     v
 [ Execute Upstream: GET /projects?org_id=org_active_123 ]
                     |
                     v
 [ Agent Receives Full Project List on Turn 1 ]
```
