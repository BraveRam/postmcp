...
```
 [ LLM Agent / MCP Client (Claude, Cursor, OpenCode) ]
                         |
...
|    - Swagger 2.0 / OpenAPI 3.0 / 3.1 parser                 |
|    - Recursive $ref resolution                              |
|    - Schema normalization & vendor extension extraction     |
|                                                             |
|  [ Tool Registry & JIT Router ]                             |
|    - Turn-1 Hot Tools pre-mounted upfront                   |
|    - Meta-tool: tool_search for dynamic discovery           |
|    - LRU eviction and memory bounds                         |
|                                                             |
|  [ Safety Shield & Security Engine ]                        |
|    - 3-Tier Risk Classifier (READ_ONLY, MUTATION, CRITICAL)  |
|    - Dry-Run Interceptor                                    |
|    - SSRF & Private Network Defense                         |
|                                                             |
|  [ Execution & Context Engine ]                             |
|    - Auto-Tenant / Identity resolver (org_id, team_id)      |
|    - Internal Auto-Pagination (anti-loop guardrail)         |
|    - Auth Handler (Bearer, ApiKey, Basic, Custom)          |
|                                                             |
|  [ Token Diet Engine ]                                      |
|    - HTML stripping                                         |
|    - Array to Markdown table transformation                 |
|    - JSONPath field masks                                   |
|    - Truncation ceilings with agent indicators              |
+-------------------------------------------------------------+
                         |
                         | HTTP/HTTPS (REST Requests)
                         v
              [ Upstream REST Services ]
```
