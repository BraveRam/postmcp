...
```
+-------------------------------------------------------------+
|               OpenAPI 2.0 / 3.0 / 3.1 Spec                  |
+-------------------------------------------------------------+
                               |
                               v
+-------------------------------------------------------------+
|                      PostMCP Engine                         |
|  - Spec Normalizer (Resolves $refs, schemas, parameters)     |
|  - Token Diet Engine (HTML strip, tables, field masks)      |
|  - Hybrid JIT Router (Hot tools upfront + tool_search)      |
|  - Safety Shield (3-tier risk classifier & dry-run)         |
|  - Auto-Context & Tenant Resolver (Inject org/team/account) |
|  - Internal Auto-Pagination (Consolidates cursors)          |
+-------------------------------------------------------------+
                               |
            +------------------+------------------+
            |                                     |
            v                                     v
+-----------------------+             +-----------------------+
|  Runtime MCP Server   |             |  Standalone Exporter  |
|  - Stdio Transport    |             |  - TypeScript MCP SDK |
|  - SSE / HTTP Streams |             |  - Python FastMCP     |
+-----------------------+             +-----------------------+
```
