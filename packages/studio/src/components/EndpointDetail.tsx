  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-wrap items-center gap-2 text-xs pt-2 border-t border-border">
            <span className="text-muted-foreground font-sans text-[11px]">Tool ID:</span>
            <Badge variant="secondary" className="font-sans text-[11px]">
              {operation.id}
            </Badge>

            <span className="text-muted-foreground font-sans text-[11px] ml-3">Risk Tier:</span>
            <Badge
              variant={
            ...
            {operation.tags.length > 0 && (
              <>
                <span className="text-muted-foreground font-sans text-[11px] ml-3">Tags:</span>
      ...
      <Card>
        ...
        <CardContent>
          {operation.parameters.length === 0 ? (
            <div className="text-xs text-muted-foreground font-sans italic p-4 bg-muted/30 rounded border border-border text-center">
            ...
            <div className="border border-border rounded-md overflow-hidden bg-card divide-y divide-border">
              {operation.parameters.map((param) => (
                <div key={`${param.in}-${param.name}`} className="p-3 text-xs">
                  ...
                  {param.description && (
                    <p className="text-muted-foreground mt-1.5 text-[11px] leading-relaxed">{param.description}</p>
                  )}
                </div>
              ))}
            </div>
      ...
      {operation.inputSchema && (
        <Card>
          ...
          <CardContent>
            <pre className="p-4 bg-muted/40 border border-border rounded-md font-sans text-xs text-foreground overflow-x-auto max-h-72">
