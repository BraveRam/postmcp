...
export function PromptInputFooter({ className, children, ...props }: PromptInputFooterProps) {
  return (
    <div
      className={cn(
        'flex items-center justify-between border-t border-border pt-2 px-1 text-xs font-sans',
        className
      )}
      {...props}
    >
...
export function PromptInputButton({
  ...
  return (
    <Button
      ...
      size="sm"
      className={cn(
        'h-7 px-2 text-xs text-muted-foreground hover:text-foreground cursor-pointer font-sans',
        className
      )}
      title={tooltip}
      {...props}
  ...
  if (isStreaming) {
    return (
      <Button
        ...
        onClick={onStop}
        className={cn(
          'h-7 w-7 rounded-md cursor-pointer font-sans',
          className
        )}
        title="Stop generation"
        {...props}
      >
        <Square className="h-3 w-3 fill-foreground" />
      </Button>
    );
  }
